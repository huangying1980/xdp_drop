...
#include "xdrop_overflow.h"

#define BUFSIZE 30
#define FIRST_PRINTABLE 65
...
static int handle_ifname(char *optarg, void *tgt, void *typearg)
{
    ...
    if (!ifindex) {
        ERR_OUT("Couldn't find network interface '%s'.\n", optarg);
        return -1;
    }
...
static int handle_subnet(char *optarg, void *tgt, void *typearg)
{
    char                *pos = NULL;
    struct subnet_addr  *subnet = tgt;
    int                  ret = 0;
    int                  prefix = 0;
    pos = strchr(optarg, '/');
    if (!pos) {
    ...
    *pos = 0;
    ...
    subnet->prefix = (uint32_t)prefix;
    if (inet_pton(AF_INET, optarg, &subnet->addr) == 1) {
        subnet->af = AF_INET;
        goto out;
    }
    if (inet_pton(AF_INET6, optarg, &subnet->addr) == 1) {
        subnet->af = AF_INET6;
        goto out;
    }
...
static void print_help_flags(const struct prog_option *opt)
{
    ...
    if (!opt->typearg)
        ERR_OUT("Missing typearg for opt %s\n", opt->name);
    else
    ...
    struct option **options, char **optstring)
{
    ...
    int                  num = 0; 
    int                  num_cmn = 0;
    int                  n_sopt = VERSION_SHORT_OPT + 1;
    char                 buf[100];
    char                *c = buf;
        ...
        if (opt->short_opt) {
            *(c++) = opt->short_opt;
            ...
            if (n_sopt >= FIRST_PRINTABLE) {
                ERR_OUT("Too many options with no short opt\n");
                goto err;
            }
...
int parse_cmdline_args(int argc, char **argv, struct prog_option *poptions,
    void *cfg, const char *prog, const char *doc, const void *defaults)
{
    ...
    FOR_EACH_OPTION (poptions, opt_iter) {
        ...
        if (opt_iter->required) {
            if (opt_iter->positional) {
                ERR_OUT("Missing required parameter %s\n",
                    opt_iter->metavar ?: opt_iter->name);
            } else {
                ERR_OUT("Missing required option '--%s'\n", opt_iter->name);
            }
            usage(prog, doc, poptions, full_help);
            err = EXIT_FAILURE;
            goto out;
...
{
    const struct prog_command *c;
    const struct prog_command *cmd = NULL;
    ...
    if (!cmd) {
        ERR_OUT("Command '%s' is unknown, try '%s help'.\n",
            argv0, prog_name);
        return EXIT_FAILURE;
    }
    ...
    cfg = malloc(cfg_size);
    if (!cfg) {
        ERR_OUT("Couldn't allocate memory\n");
        return EXIT_FAILURE;
    }
    ...
    snprintf(pin_root_path, sizeof(pin_root_path), "%s/%s", pin_base_dir, PROG_NAME);
